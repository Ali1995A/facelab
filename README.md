...
- 特效强化为高强度卡通风（更大粒子、更高密度、更鲜艳）
- 特效按钮容器放大，点击区域更大
- 文字特效选项（经典/霓虹/火焰/糖果/抖动）
- 文字可在画布中拖动摆放
- 实时预览像素优化（减少马赛克，保留轻像素风）
- 实时画面提亮与去灰优化（轻暖色调+增饱和）
- 圆形快门：短按拍照、长按录像（绿色进度环，最长10秒）
- 第一代 iPad Pro（Chrome）流畅度优化：低功耗渲染档、切屏防抖、保存减负
...
- iPad Chrome 低功耗档会自动启用：限制帧率、降低粒子上限、切屏重排防抖

## 儿童模式操作

- 短按圆形快门：拍照
- 长按圆形快门：开始录像并锁定，绿色进度环最多 10 秒（可松手继续录）
- `➕字` 添加常用文字（可拖动到任意位置）
- 选项条可切换画面特效与文字特效（录制中也可切换）
- 支持先选特效再拍摄
- 支持先拍摄再选特效，`✅完成` 时自动合成并保存
- 录制结束方式：点 `✅完成` 或等待 10 秒进度走满
- `✅完成` 自动保存到相册（优先系统分享，失败则自动下载）
- 保存成功后自动进入下一轮（无需手动重置）

## 维护提示

- UI 流程状态由 `setUiMode()` 统一控制（`capture` / `recording` / `edit` / `saved`），并同步顶部 flow 芯片。
- 长按录制入口在 `onShutterPointerDown()`，锁定逻辑在 `startRecording({ locked: true })`。
- 保存后自动重置流程在 `finalizeSaveAndReset()`。
- 第一代 iPad Pro 流畅度参数集中在 `state.performance` 的低功耗分支，可按机型继续微调帧率、粒子、码率。
